...
import com.cs389.whatsaround.adapter.QuestionsAdapter;
...
public class SettingsActivity extends ActionBarActivity implements AdapterView.OnItemClickListener, AbsListView.MultiChoiceModeListener, DeletionDialog.DeletionDialogListener {
    ...
    private List<Question> questionsListed = Question.getQuestionsSample(); //?????????? CHECK IF IT'S GOOD PRACTICE ????????
    //It's going to change for a list that come from the database
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        TextView imageView = (TextView) view.findViewById(R.id.txt_image);
        ...
        Question question = new Question(imageView.getText().toString(), answerView.getText().toString());
